    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-100 p-6">
        <div className="max-w-md w-full p-8 bg-white rounded-lg shadow-lg text-center">
          <h1 className="text-4xl font-semibold mb-6 text-gray-800">Access Denied</h1>
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-6">
      <div className="flex items-center space-x-6 mb-8">
        ...
        <div>
          ...
          {newBio && (
            <p className="text-gray-600 text-lg mt-2 cursor-pointer" onClick={openBioModal}>
              {bioDisplay}
            </p>
          )}
        </div>
      ...
      {editing && (
        <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
          <h3 className="text-xl font-medium mb-4 text-gray-800">Edit Profile</h3>
          <input
            type="text"
            placeholder="New display name"
            value={newDisplayName}
            onChange={(e) => setNewDisplayName(e.target.value)}
            className="border border-gray-300 rounded-md p-3 w-full mb-4 focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
          />
          <textarea
            placeholder="New bio"
            value={newBio}
            onChange={(e) => setNewBio(e.target.value)}
            className="border border-gray-300 rounded-md p-3 w-full mb-4 h-24 focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
          />
          <button
            onClick={handleSave}
      ...
      >
        <h3 className="text-xl font-medium mb-4 text-gray-800">Full Bio</h3>
        <p>{newBio}</p>
        <button
